...
- Nixpkgs-Quellcode, `nixos/modules/services/misc/sssd.nix` (release-26.05) – Option `services.sssd.environmentFile` samt Options-Beschreibung ("Secrets may be passed to the service without adding them to the world-readable Nix store …") und dem Beispiel `ldap_default_authtok = $SSSD_LDAP_DEFAULT_AUTHTOK`; im Unit `EnvironmentFile = lib.mkIf (cfg.environmentFile != null) cfg.environmentFile;` mit dem Quellcode-Kommentar "We cannot use LoadCredential here because it's not available in ExecStartPre"; `preStart` mit `envsubst` unter `umask 0177` – https://github.com/NixOS/nixpkgs/blob/release-26.05/nixos/modules/services/misc/sssd.nix
- sops-nix (GitHub), `modules/sops/default.nix` – Option `sops.secrets.<name>.restartUnits` sowie `systemd.services.sops-install-secrets` mit `wantedBy = [ "sysinit.target" ]` (Secrets liegen vor dem Start normaler Dienste) – https://github.com/Mic92/sops-nix/blob/master/modules/sops/default.nix
- systemd-Quellcode, `man/systemd.exec.xml`, Abschnitt `EnvironmentFile=` – Reihenfolge mehrerer Einträge, wörtlich: "If the same variable is set twice from these files, the files will be read in the order they are specified and the later setting will override the earlier setting." – https://github.com/systemd/systemd/blob/main/man/systemd.exec.xml
