...
- **`sops.secrets.<name>.restartUnits`**: Liste von Units, die neu gestartet werden, wenn sich der Inhalt eines Secrets ändert – funktioniert wie `systemd.services.<name>.restartTriggers`. Ohne diesen Eintrag läuft ein Dienst nach einem Passwortwechsel unbemerkt mit dem alten Wert weiter.
- **`services.sssd.environmentFile`**: Weg, Geheimnisse in die `sssd.conf` zu bekommen, ohne sie in den Store zu schreiben. In der Nix-Konfiguration stehen nur Platzhalter (`ldap_default_authtok = $SSSD_LDAP_DEFAULT_AUTHTOK`); der `preStart` des Dienstes ersetzt sie per `envsubst` zur Laufzeit. Im Store liegt dauerhaft nur die Platzhalter-Fassung, die substituierte Datei entsteht unter `/var/lib/sssd/` mit `umask 0177`.
- **`envsubst`-Leerersetzung (Fehlerbild)**: Fehlt eine referenzierte Variable in der Environment-Datei oder ist sie leer, ersetzt `envsubst` den Platzhalter kommentarlos durch nichts. `sssd` bindet dann mit leerem Passwort – der Login scheitert ohne Hinweis auf die eigentliche Ursache.
- **Mehrere `EnvironmentFile=`-Einträge**: systemd liest sie in der angegebenen Reihenfolge; bei gleichnamigen Variablen gewinnt der spätere Eintrag. Erlaubt es, eine unverschlüsselte Basis-Datei mit unkritischen Werten und eine sops-entschlüsselte Datei mit Geheimnissen nebeneinander zu binden, statt alles in eine verschlüsselte Datei zu zwingen.
